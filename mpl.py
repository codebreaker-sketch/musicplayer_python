    ...
    if path:
        ...
        for song in songs:
            if song.endswith(".mp3"):
                playlist.insert(END, os.path.join(path, song))

def play_song():
    selected_song = playlist.get(ACTIVE)
    mixer.music.load(selected_song)
    mixer.music.play()
    music_name = os.path.basename(selected_song)
...
def delete_song():
    ...
    if selected_song_index:
        ...
        # Stop the music if the currently playing song is deleted
        if mixer.music.get_busy() and selected_song == playlist.get(ACTIVE):
            mixer.music.stop()
            music.config(text="")
...
Label(root, image=Menu, bg="#f01a2b").pack(padx=10, pady=50, side=RIGHT)

music_frame = Frame(root, bd=2, relief=RIDGE)
music_frame.place(x=330, y=350, width=560, height=250)

Button(root, text="Open Folder", width=15, height=2, font=("arial", 10, "bold"), fg="white", 
       bg="#21b3de", command=open_folder).place(x=330, y=300)

delete_button = Button(root, text="Delete Song", width=15, height=2, font=("arial", 10, "bold"), 
                       fg="white", bg="#de2121", command=delete_song)
delete_button.place(x=500, y=300)
